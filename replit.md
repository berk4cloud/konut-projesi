...
-   **Tenant Timezone Support**: Multi-timezone architecture with IANA timezone configuration per tenant, default Europe/Amsterdam. Settings page provides timezone selector with 17 major timezone options (Europe, Americas, Asia, Australia) with UTC offset display. All date calculations (reservations, checkouts, availability) respect tenant's local timezone using Luxon. Features include automatic normalization (whitespace trimming), strict validation (create + update), runtime UTC fallback, and shared timezone helpers for consistent date handling across the system.
...
-   **Multi-Level Pricing System**: Supports a 3-level pricing hierarchy (Room > House > System) with both daily and monthly pricing for each level. The system persists both `costPerDay` and `costPerMonth` in the database and auto-enables custom pricing when users enter values without requiring manual checkbox interaction.
